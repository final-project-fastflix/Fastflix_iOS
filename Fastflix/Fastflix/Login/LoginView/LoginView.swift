...
//  Fastflix
//
//  Created by HongWeonpyo on 14/08/2019.
...
class LoginView: UIView {
  ...
  private func configure() {
    self.backgroundColor = #colorLiteral(red: 0.07450980392, green: 0.07450980392, blue: 0.07450980392, alpha: 1)
    emailTextField.delegate = self
    passwordTextField.delegate = self
  }
  
  private func addSubViews() {
    [navigationView, stackView, passwordResetButton].forEach { addSubview($0) }
  }
  ...
  @objc private func didReceiveKeyboardNotification(_ noti: Notification) {
    ...
    let overlappedHeight = (textViewHeight*3 + 36 + 10)/2 - (self.frame.height/2 - keyboardFrame.height)
    ...
    if keyboardFrame.minY >= self.frame.maxY {
      // 스택뷰의 오토레이아웃 업데이트
